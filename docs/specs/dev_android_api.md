...
- `apiRoot` 未配置时，登录走 Android 原生网易云登录 provider；推荐、发现、歌单、搜索、歌词、评论等非登录远程 API 不主动请求
- `apiRoot` 已配置时，登录继续走外部 `API_ROOT/netease/*`，且不实例化原生登录 provider
- 本地登录只在用户触发登录、刷新或登出时按需请求，不启动 Node、本地 HTTP server 或桌面 runtime
...
- `apiRoot` 为空时，登录走 Android 原生网易云登录 provider，不要求用户先配置外部服务器
- 原生登录接口面按 desktop/standalone 登录 API 对齐：二维码、登录态、刷新、登出、短信验证码、手机号登录、国家码列表
- 设置页初始化不自动请求原生登录端点；只有点击二维码、发送验证码、手机号登录、刷新登录或退出登录时才请求
- 二维码轮询由设置页可见状态持有，取消登录、二维码过期、登录成功或 ViewModel 清理时必须停止
- `apiRoot` 已配置时，登录继续走外部 `API_ROOT/netease/*`，且不创建原生登录客户端
